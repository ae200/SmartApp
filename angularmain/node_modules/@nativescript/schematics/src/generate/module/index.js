...
function default_1(options) {
    ...
    return schematics_1.branchAndMerge(schematics_1.chain([
        ...
        () => {
            const opts = utils_2.removeNsSchemaOptions(options);
        ...
        (tree) => shouldCreateCommonFile(platformUse, options.common) ?
            schematics_1.schematic('common-module', { name: moduleInfo.name, path: moduleInfo.path }) :
            tree,
    ]));
}
...
const parseModuleInfo = (tree, options) => {
    ...
    if (!moduleInfo.moduleFilePath) {
        throw new schematics_1.SchematicsException(`Failed to find generated module files from @schematics/angular. ` +
...
const addSchema = (modulePath) => (tree) => {
    const moduleSource = ts_utils_1.getSourceFile(tree, modulePath);
...
const removeNGCommonModule = (tree, modulePath) => {
    ...
    ts_utils_1.removeImport(tree, modulePath, moduleName);
    ts_utils_1.removeMetadataArrayValue(tree, modulePath, 'imports', moduleName);
    return tree;
};
