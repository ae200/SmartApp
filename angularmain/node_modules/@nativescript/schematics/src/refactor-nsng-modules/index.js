    ...
    return schematics_1.chain([
        (tree) => {
            ...
            tree.visit((path) => {
                ...
                const ngModules = ts_utils_1.getDecoratedClasses(tree, path, 'NgModule');
                const metadataObjects = ngModules
                    .map((m) => ({
                    metadataObject: ts_utils_1.getDecoratorMetadataFromClass(m, 'NgModule'),
                    classNode: m,
                }))
            ...
            if (animationModuleIsUsed) {
                const rootModuleMetadata = ts_utils_1.getDecoratorMetadataFromClass(rootModule, 'NgModule');
...
const getBootstrappedModule = (tree, path, sourceDir) => {
    const entrySource = ts_utils_1.getSourceFile(tree, path);
    const bootstrappedModules = ts_utils_1.collectDeepNodes(entrySource, ts.SyntaxKind.CallExpression)
        .filter((node) => ts_utils_1.filterByChildNode(node, (child) => child.kind === ts.SyntaxKind.PropertyAccessExpression &&
    ...
    const imports = ts_utils_1.findImports(moduleName, entrySource);
    ...
    const rootModule = ts_utils_1.getDecoratedClasses(tree, rootModulePath, 'NgModule')
        .find((c) => !!(c.name && c.name.getText() === moduleName));
    return { rootModule, rootModulePath };
};
const refetchMetadata = (tree, path, classNode) => {
    const newClassNode = ts_utils_1.getDecoratedClass(tree, path, 'NgModule', classNode.name.getText());
    const newMetadataObject = ts_utils_1.getDecoratorMetadataFromClass(newClassNode, 'NgModule');
    return newMetadataObject;
};
const importNgModule = (tree, path, metadataObject, name, importPath) => {
    const nodesToAdd = ts_utils_1.getSymbolsToAddToObject(path, metadataObject, 'imports', name);
...
const removeImportedNgModule = (tree, path, metadataObject, name) => {
    ...
    if (removed) {
        ts_utils_1.removeImport(tree, path, name);
    }
    return removed;
};
const removeNgModuleFromMetadata = (tree, path, metadataObject, name) => {
    const metadataImports = ts_utils_1.getNodesToRemoveFromNestedArray([metadataObject], 'imports', name);
